...
ハイパーパラメータ 0。SVD 1回。CPU のみ。最先端の精度。

- **[Chronos Benchmark II](https://github.com/amazon-science/chronos-forecasting) で第1位** (25データセット・ゼロショット) — Agg. Rel. MASE **0.696**、全Foundation Model (最大710Mパラメータ, GPU) を上回る
...
Shape は構造的なもの（学習されない）なので、過学習しません。Level は圧縮された滑らかな系列 — P個の値の代わりに1周期あたり1つの値 — を Ridge 回帰で予測します。この二重の圧縮（加算によるノイズ低減 + 予測ステップ数の H → H/P への削減）が、FLAIRが数十億パラメータのモデルと競合できる理由です。
...
FLAIR は依存2つの単一ファイルです。pip パッケージは未公開 — コピーまたは clone してください：
...
ベースライン結果: [autogluon/fev](https://github.com/autogluon/fev)、[amazon-science/chronos-forecasting](https://github.com/amazon-science/chronos-forecasting)。FLAIR は Chronos-T5-Small (46Mパラメータ) に対して **25データセット中14勝**。
...
FLAIR は **8データセット中3勝**、**32個別設定中11勝**。明確な周期性を持つデータセットに強く、非周期系列 (Exchange) では弱い。

### なぜ FLAIR は動くのか？

3つの同時圧縮が FLAIR の競争力ある精度を説明します：

1. **ノイズ低減**: P個の位相を1つの Level 値に加算することで、ノイズが ~√P 倍低減
2. **ホライゾン圧縮**: Level の予測は H ステップではなく ⌈H/P⌉ ステップのみで済み、誤差蓄積を軽減
3. **過学習免疫**: Shape は構造的（Dirichlet事後分布）で学習されないため、過学習が原理的に不可能
...
| メソッド | 説明 |
|---------|------|
| `predict(y, horizon, n_samples=None)` | `forecast()` と同等、インスタンスのデフォルト値を使用 |
